...
- PixiJS 기반 소환/합성/판매/이동/웨이브/공격 연출

React는 홈, 대시보드, 랭킹, 프로필, 관리자 같은 서비스 UI를 담당하고, PixiJS는 실제 게임 플레이 화면을 담당합니다.

## 현재 우선순위

현재는 로그인/랭킹/API보다 **게임 기본 재미 구현**을 우선합니다.

1차 게임 개발 목표:

- 적이 실제로 길을 따라 이동하는 자동 웨이브 구조
- 유닛이 적을 자동 타겟팅하고 투사체로 공격하는 전투 연출
- 적 개체별 HP, 처치 보상, 누수, 코어 HP 감소 처리
- 보스 웨이브 특수 경고/연출
- 웨이브 보상과 행운석 지급
- 신화 조합 메뉴와 실제 조합 조건 구현
- PixiJS 렌더링 객체를 매번 재생성하지 않고 재사용하는 구조
- `createPixiGame.ts` 비대화를 줄이기 위한 기능별 모듈 분리

다음 구조 개선 목표는 `createPixiGame.ts`에 남아 있는 전투/웨이브 런타임을 별도 파일로 분리하는 것입니다.
...
5. 먼저 게임 기본 재미를 만들고, 이후 로그인/저장/랭킹을 붙인다.
6. 코드와 데이터 파일을 기능별로 분리한다.
7. 게임 밸런스 값은 코드에 하드코딩하지 않고 설정 파일 또는 DB로 분리한다.
8. 디스코드 커뮤니티 활동과 게임 보상이 자연스럽게 연결되도록 설계한다.
9. 기존에 동작하던 클라이언트를 새로 만들지 않고, 기존 클라이언트 위에서 부분 패치한다.
10. PixiJS 객체는 가능한 한 최초 생성 후 재사용하고, 값만 갱신한다.
...
- Cloudflare D1 생성 및 테이블 생성 완료
- GitHub Actions 기반 Workers 자동 배포 성공 확인
- `packages/game` 순수 게임 로직 패키지 구성 중
- PixiJS `/play` 독립 게임 화면 프로토타입 구현 중
- 자동 웨이브, 적 이동, 유닛 자동 공격, 보스 웨이브 경고, 행운석 보상, 신화 조합 메뉴 구현 진행
- HUD, Controls, EnemyView, BoardView 렌더링 모듈 분리 및 기존 클라이언트 연결 완료
- 다음 작업은 전투/웨이브 런타임 분리와 타입체크/배포 확인
