...
현재는 **이미 연결된 게임 플레이, Discord OAuth, 기록 저장, 랭킹 흐름을 안정화**하는 단계입니다.

우선순위:

- `pnpm build:web`, `pnpm typecheck` 기준 빌드 안정화
...
- `durationSeconds` 실제 측정 및 서버 기록 검증 추가
...
- 5x4 보드, 자동 웨이브, 적 이동, 유닛 자동 공격, 보스 웨이브 경고, 행운석 보상, 신화 조합 메뉴 구현
- HUD, Controls, EnemyView, BoardView, CombatRuntime, WaveRuntime 등 렌더링/런타임 모듈 분리 진행
