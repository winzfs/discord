...
export function updatePixiControlsView(view: PixiControlsView, layout: GameLayout, snapshot: ControlsSnapshot, force = false) {
  ...
  const shouldAutoStartWave = snapshot.wavePhase === "result" && !snapshot.waveDisabled;
  view.wave.root.visible = false;
  view.waveBacking.visible = false;
  if (!shouldAutoStartWave) return;
