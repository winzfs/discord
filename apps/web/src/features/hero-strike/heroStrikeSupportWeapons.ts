...
function spawnMissile(state: HeroStrikeState) {
  ...
  state.missiles.push({
    ...
    speed: 340,
    turnRate: 5.8,
    damage: 85 + state.player.damage * 2.2,
    radius: 7,
    explosionRadius: 52,
    life: 4.5,
    targetId: target.id,
...
function spawnDroneVolley(state: HeroStrikeState) {
  ...
  for (const side of [-1, 1] as const) {
    state.bullets.push({
      ...
      vy: -830,
      radius: 3.4,
      damage: player.damage * 0.58,
      pierce: 0,
      enemy: false,
      life: 1.35,
      color: HERO_STRIKE_COLORS.lime,
    });
  }
}
...
function updateMissiles(state: HeroStrikeState, dt: number) {
  for (const missile of state.missiles) {
    ...
    const directTarget = state.enemies.find((enemy) => !enemy.dead && distanceSquared(missile.x, missile.y, enemy.x, enemy.y) <= (missile.radius + enemy.radius) ** 2);
