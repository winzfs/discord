...
export function drawHeroStrikeResult(ctx: CanvasRenderingContext2D, state: HeroStrikeState) {
  ...
  ctx.fillText(victory ? "ALL 10 STAGES COMPLETE" : `OPERATION ENDED · STAGE ${reachedStage}`, HERO_STRIKE_WIDTH / 2, 104);

  ctx.fillStyle = gradeColor(grade);
  ctx.font = "1000 68px system-ui";
  ctx.fillText(grade, HERO_STRIKE_WIDTH / 2, 180);
  ctx.fillStyle = HERO_STRIKE_COLORS.white;
  ctx.font = "1000 23px system-ui";
  ctx.fillText(victory ? "완전 승리" : "작전 종료", HERO_STRIKE_WIDTH / 2, 212);

  roundedRect(ctx, 42, 238, HERO_STRIKE_WIDTH - 84, 264, 20);
  ctx.fillStyle = "rgba(7,20,39,.9)";
  ctx.fill();
  ctx.strokeStyle = "rgba(105,231,255,.2)";
  ctx.stroke();

  drawStat(ctx, "SCORE", state.score.toLocaleString(), 104, 268);
  drawStat(ctx, "KILLS", String(state.kills), 210, 268);
  drawStat(ctx, "MAX COMBO", String(state.maxCombo), 316, 268);
  drawStat(ctx, "DAMAGE", Math.round(state.damageDealt).toLocaleString(), 104, 326);
  drawStat(ctx, "HITS", String(state.hitsTaken), 210, 326);
  drawStat(ctx, "BLINKS", String(state.blinksUsed), 316, 326);
  drawStat(ctx, "PULSE RUSH", String(state.flowActivations), 104, 384);
  drawStat(ctx, "BOSS BREAK", String(state.bossBreaks), 210, 384);
  drawStat(ctx, "PERFECT", String(state.perfectStages), 316, 384);
  drawStat(ctx, "OBJECTIVES", `${state.objectivesCompleted}/10`, 104, 442);
  drawStat(ctx, "REROLLS", String(state.rerollsUsed), 210, 442);
  drawStat(ctx, "STAGE", `${reachedStage}/10`, 316, 442);

  ctx.fillStyle = HERO_STRIKE_COLORS.xp;
  ctx.font = "900 11px system-ui";
  ctx.fillText(`RESEARCH DATA +${state.runResearchEarned} · RANK ${state.researchRank}`, HERO_STRIKE_WIDTH / 2, 488);
  ...
  ctx.fillText(getLoadoutSummary(state.loadout), HERO_STRIKE_WIDTH / 2, 553);

  roundedRect(ctx, 92, 582, HERO_STRIKE_WIDTH - 184, 64, 20);
  ctx.fillStyle = HERO_STRIKE_COLORS.orange;
  ctx.fill();
  ctx.fillStyle = "#111827";
  ctx.font = "900 18px system-ui";
  ctx.fillText("장비 재정비", HERO_STRIKE_WIDTH / 2, 621);
  ctx.fillStyle = HERO_STRIKE_COLORS.muted;
  ctx.font = "700 10px system-ui";
  ctx.fillText("화면을 눌러 출격 준비로 이동", HERO_STRIKE_WIDTH / 2, 674);
  ctx.textAlign = "left";
}
