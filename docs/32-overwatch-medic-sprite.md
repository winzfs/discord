# 32. 오버워치 의무요원 medic.png 스프라이트 적용

## 1. 목적

`apps/web/public/assets/heroes/medic.png`에 추가된 오버워치 의무요원 스프라이트 시트를 로비와 전투에 모두 적용했습니다.
...
`SPRITE_HERO_IDS`에도 `mini-mender`를 추가해서 이미지 렌더링 대상에 포함했습니다.

## 4. 공격 모션 적용

`pixiCombatRuntime.ts`의 `SPRITE_ATTACK_HERO_IDS`에 `mini-mender`를 추가했습니다.

따라서 오버워치 의무요원도 공격 시 다음 처리를 탑니다.
...
- 로비 영웅 카드에서 오버워치 의무요원이 글자 대신 이미지로 보이는지 확인
- 로비 카드에서 전체 시트가 아니라 첫 대기 행만 보이는지 확인
- 전투 보드에서 오버워치 의무요원이 이미지로 보이는지 확인
- 오버워치 의무요원이 공격 시 공격 행으로 바뀌는지 확인
