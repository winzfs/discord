...
현재 웨이브는 몬스터 이동 연출 후 자동으로 완료되는 프로토타입에 가깝습니다.

바로 실시간 전투 전체를 구현하기보다, 다음 순서로 개발합니다.

### 5.1 1차 전투 판정: 전투력 기반 웨이브 판정

목표는 소환과 합성이 실제 생존에 영향을 주게 만드는 것입니다.
...
2. 웨이브의 총 위협도를 계산한다.
3. 전투력이 충분하면 웨이브를 클리어한다.
4. 전투력이 부족하면 일부 적이 새고 코어 HP가 감소한다.
5. 남은 코어 HP가 0이면 실패 처리한다.

이 단계에서는 아직 투사체, 개별 적 HP, 실시간 타겟팅을 완성하지 않습니다.

### 5.2 2차 전투 연출: 전투감을 먼저 추가
