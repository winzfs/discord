...
초기에는 `createPixiGame.ts`에 대부분의 코드가 있었지만, 현재는 렌더링 관련 기능을 단계적으로 분리했습니다.
...
2. 각 영웅 역할군에 따라 타겟을 선택합니다.
3. 투사체를 생성해 유닛 위치에서 적 위치로 이동시킵니다.
4. 투사체 도착 시 적 HP를 감소시킵니다.
5. 적 HP가 0이 되면 처치 처리하고 코인 보상을 지급합니다.
6. 탱커는 감속, 지원가는 주변 스플래시 보조 피해를 적용합니다.

현재 이 전투 로직은 아직 `createPixiGame.ts` 내부에 남아 있으므로, 다음 단계에서 `pixiCombatRuntime.ts`로 분리하는 것이 우선입니다.
...
- 행운석은 도박 소환에 사용
- 신화 조합은 행운석 구매 방식이 아니라 레시피 재료 기반으로 진행
...
또한 최근 보드 모듈 연결 후에는 반드시 다음 확인이 필요합니다.
...
1. `pnpm typecheck` 기준으로 타입 오류 정리
2. `/play` 실제 실행 확인
3. 최근 보드 모듈 연결 후 유닛 렌더링/드래그/합성/판매 회귀 테스트
4. 전투/타겟팅/투사체 로직을 `pixiCombatRuntime.ts`로 분리
5. 웨이브 시작/종료/보상 로직을 `pixiWaveRuntime.ts`로 분리
6. 신화 조합창을 `pixiMythicMenuView.ts`로 분리
7. 배경/길 렌더링을 별도 뷰로 분리
